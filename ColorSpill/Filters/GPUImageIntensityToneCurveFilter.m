//
//  GPUImageIntensityToneCurveFilter.m
//  ColorSpill
//
...
/*
 ...
 one the ability to modify the intensity of the original effect by adjusting the intensity property.
...
@interface GPUImageACVFile2 : NSObject{
    short version;
    short totalCurves;
...
unsigned short int16WithBytes2(Byte* bytes);
@end

@implementation GPUImageACVFile2
...
- (id) initWithACVFileData:(NSData *)data {
    ...
    if (self != nil)
    {
        ...
        version        = int16WithBytes2(rawBytes);
        rawBytes+=2;
        
        totalCurves    = int16WithBytes2(rawBytes);
        rawBytes+=2;
        ...
        {
            unsigned short pointCount = int16WithBytes2(rawBytes);
            rawBytes+=2;
            ...
            {
                unsigned short y = int16WithBytes2(rawBytes);
                rawBytes+=2;
                unsigned short x = int16WithBytes2(rawBytes);
                rawBytes+=2;
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
NSString *const kGPUImageToneCurveFragmentShaderString2 = SHADER_STRING
(
 ...
 void main()
 {
     ...
     gl_FragColor = mix(textureColor, vec4(redCurveValue, greenCurveValue, blueCurveValue, 1.0), intensity);
 }
 );
#else
NSString *const kGPUImageToneCurveFragmentShaderString2 = SHADER_STRING
...
@interface GPUImageIntensityToneCurveFilter()
{
...
@implementation GPUImageIntensityToneCurveFilter
...
- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageToneCurveFragmentShaderString2]))
    {
		return nil;
    }
...
- (id)initWithACVData:(NSData *)data {
    ...
    GPUImageACVFile2 *curve = [[GPUImageACVFile2 alloc] initWithACVFileData:data];
...
{
    NSData* fileData = [NSData dataWithContentsOfURL:curveFileURL];
    GPUImageACVFile2 *curve = [[GPUImageACVFile2 alloc] initWithACVFileData:fileData];
